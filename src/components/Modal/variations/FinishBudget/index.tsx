...
type Props = {
  data?: any
  onClose?: () => void
  handleOp?: (reason: string) => Promise<void>
}

const FinishBudget = ({ onClose, handleOp }: Props) => {
  ...
  const handleSubmit = async () => {
    ...
    if (!errorsCheck.has) {
      if (handleOp) await handleOp(form.reason)
      clearFields()
    } else {
    ...
    <S.Element>
      <C.Header>
        <C.HeaderDefault>
          <C.HeaderMain>
            <C.ModalTitle>
              Por que não contratou uma empresa participante?
            </C.ModalTitle>
            {onClose && (
      ...
      <S.Content>
        ...
        <S.Bottom>
          ...
          <Button
            type="main"
            text={submitting ? "Finalizando..." : "Finalizar"}
